 ...
 */
package in.andonsystem.threads;
 ...
 */
public class ScheduleSolvedThread extends Thread{
    ...
    public ScheduleSolvedThread(int issueId,int line,int probId,String message,int level){
    ...
    @Override
    public void run() {
        System.out.println("\nTime: " + new Date(System.currentTimeMillis()) + ",ScheduleSolvedThread.run() for issuedId :" + issueId + " , and level:" +level);
        ...
        if(!result){
            ...
            if(level == 2 && seekValue == 0){       //Send SMS to user Level 2 only if Level 1 hasn't already sought help
                list = desgnService.getDesgnConcerned(line, probId, 2);
            }
            if(level == 3 && seekValue != 2){       //Send SMS to user Level 3 only if Level 2 hasn't already sought help
